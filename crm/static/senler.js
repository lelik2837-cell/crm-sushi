(() => {
  ...
  function channelForm(id) {
    ...
    document.getElementById('sn-channel-form').onsubmit = async event => {
      ...
      try {
        const data = {name:document.getElementById('ch-name').value,token:document.getElementById('ch-token').value};
  ...
  function renderCampaignEditor() {
    ...
    main.innerHTML=navLink('campaigns','<i class="bi bi-arrow-left"></i> К рассылкам',{},'sn-back')+head(c.id?'Редактирование рассылки':'Новая рассылка','Четыре понятных шага до отправки',btn('Сохранить черновик','campaign-save'))+`<div class="sn-panel"><ol class="sn-wizard">${['Аудитория','Сообщение','Время','Проверка'].map((s,i)=>`<li class="${i===state.wizard?'active':''}"><b>${i+1}</b>${s}</li>`).join('')}</ol><div class="sn-editor-layout"><div>${content}</div><div id="sn-campaign-preview">${preview(c.body,c.audience.channels.length?channelName(c.audience.channels[0]):'Ваше сообщество')}</div></div><div class="sn-form-footer"><div>${state.wizard?btn('← Назад','campaign-prev'):''}</div>${state.wizard<3?btn('Далее →','campaign-next','',true):btn(c.sendMode==='schedule'?'Запланировать рассылку':'Запустить рассылку','campaign-launch','id="sn-launch" disabled',true)}</div></div>`;
    main.querySelectorAll('input,textarea,select').forEach(e=>e.addEventListener('input',()=>{state.dirty=true;collectCampaign();if(state.wizard===1){document.getElementById('sn-campaign-preview').innerHTML=preview(c.body,channelName(c.audience.channels[0]));const count=main.querySelector('[data-text-count]');count.textContent=`${c.body.text.length} / ${c.body.asset_id?900:3500}`;}}));
