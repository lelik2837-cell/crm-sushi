(() => {
  ...
  const EMOJIS = ['😀','😊','😉','😍','🥳','😋','👍','🙏','🎉','🔥','⭐','❤️','✅','⏰','📢','🎁','💥','🍣','🍱','🍜','🍙','🥢','🍤','🐟','🍚','🌶️','🥤','🚀','📍','💬'];
  const BUTTON_COLORS = [['secondary','Обычная'],['primary','Синяя'],['positive','Зелёная'],['negative','Красная']];
  ...
  const profileUrl = s => s.kind === 'vk' ? 'https://vk.com/' + (s.username || ('id' + s.external_user_id)) :
    s.kind === 'telegram' ? (s.username ? 'https://t.me/' + s.username : '') :
    s.kind === 'max' ? (s.username ? 'https://max.ru/' + s.username : '') : '';
  const avatarCell = s => {
    const src = s.avatar_url || (s.avatar_file ? apiBase + 'subscribers/' + s.id + '/avatar' : '');
    const initial = esc((s.name || s.username || '?').trim().charAt(0).toUpperCase() || '?');
    const img = src ? `<img class="sn-avatar" src="${esc(src)}" alt="" loading="lazy" onerror="this.remove()">` : '';
    return `<span class="sn-avatar-wrap"><span class="sn-avatar sn-avatar-fallback">${initial}</span>${img}</span>`;
  };
