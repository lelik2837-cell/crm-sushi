...
def init_db():
    ...
    with get_db() as conn:
        ...
        # Одноразовая очистка: RFM-блоки дашборда (были кратко задеплоены и отменены —
        # пользователь попросил убрать) успели засеять в user_dashboard_blocks строки
        # 'rfm_recency'/'rfm_avg_check'/... для тех, кто хоть раз открыл /dashboard за это
        # время; ключей 'rfm_*' в реестре DASHBOARD_REVENUE_BLOCKS больше нет, но сами
        # строки без этой чистки остались бы в базе и фантомно всплывали бы в списке
        # настройки видимости/порядка блоков (settings.html) с сырым ключом вместо
        # названия. Sort_order у 'delivery_time' не трогаем обратно — он сдвигался
        # относительным образом, финальный порядок (последний блок) не меняется от
        # конкретного числа, а надёжно откатить его отдельно от возможной ручной
        # перестановки блоков пользователем за это время нельзя.
        if not conn.execute(
            "SELECT 1 FROM api_settings WHERE key='dashboard_blocks_rfm_removed_v1'"
        ).fetchone():
            conn.execute("DELETE FROM user_dashboard_blocks WHERE block_key LIKE 'rfm\\_%' ESCAPE '\\'")
            conn.execute(
                "INSERT OR REPLACE INTO api_settings (key, value) VALUES ('dashboard_blocks_rfm_removed_v1', '1')"
            )
...
def _rating_broadcast_stats(conn, date_from, date_to, branch_ids):
    """Статистика по дням для вкладки «Статистика» (Рассылка → Оценка заказа).

    Группировка — по дню СОЗДАНИЯ заявки (order_rating_requests.created_at), а не по дню
    отправки: просьба пользователя 2026-09-05 показать в таблице «по дням» колонку «Всего
    номеров» — то есть сколько заявок вообще заведено за день (включая ещё не отправленные —
    pending_send, пропущенные — skipped, см. п.324, и неотправленные — failed), а не только
    те, что реально ушли. «Отправлено» — подмножество «Всего» (sent_at IS NOT NULL). Разбивка
    по каналу (messenger_accounts.channel через assigned_account_id) — только среди
    отправленных, у ещё не отправленных заявок канала попросту нет. «Ответили» и его доля
    считаются от ВСЕХ номеров за день (столбец 1), а не от отправленных — так и попросил
    пользователь: «сколько ответили разделить на все номера, в процентах».

    created_at/sent_at пишутся через datetime('now')/CURRENT_TIMESTAMP (UTC) — переводим в
    локальное время (+7 часов), тем же способом, что и в остальном коде вкладки «Рассылка»
    (см. rating_requests в broadcast_page()) и в отчёте «Отзывы» (см. п.314/319/324 в
    plan.md — тот же класс бага)."""
    branch_filter_sql = ''
    branch_params = []
    if branch_ids:
        ph = ','.join('?' * len(branch_ids))
        branch_filter_sql = f' AND rc.branch_id IN ({ph})'
        branch_params = list(branch_ids)

    date_filter_sql = (
        "date(datetime(rr.created_at, '+7 hours')) >= ? AND date(datetime(rr.created_at, '+7 hours')) <= ?"
    )

    totals_rows = conn.execute(f'''
        SELECT date(datetime(rr.created_at, '+7 hours')) AS day,
               COUNT(*) AS total_count,
               SUM(CASE WHEN rr.sent_at IS NOT NULL THEN 1 ELSE 0 END) AS sent_count,
        ...
        WHERE {date_filter_sql}{branch_filter_sql}
        GROUP BY day
    ''', [date_from, date_to] + branch_params).fetchall()

    # Отдельным запросом — разбивка ОТПРАВЛЕННЫХ по каналу (INNER JOIN на messenger_accounts,
    # у неотправленных заявок assigned_account_id пуст, они сюда не попадают вообще).
    channel_rows = conn.execute(f'''
        ...
        WHERE rr.sent_at IS NOT NULL AND {date_filter_sql}{branch_filter_sql}
    ...
    for r in totals_rows:
        by_day[r['day']] = {
            'day': r['day'], 'total': r['total_count'], 'sent': r['sent_count'],
            'responded': r['responded_count'], 'by_channel': {}
        }
    for r in channel_rows:
        entry = by_day.setdefault(r['day'], {'day': r['day'], 'total': 0, 'sent': 0, 'responded': 0, 'by_channel': {}})
        entry['by_channel'][r['channel']] = {'sent': r['sent_count'], 'responded': r['responded_count']}

    days = sorted(by_day.values(), key=lambda d: d['day'])
    for d in days:
        d['sent_pct'] = round(d['sent'] / d['total'] * 100, 1) if d['total'] else 0
        d['responded_pct'] = round(d['responded'] / d['total'] * 100, 1) if d['total'] else 0

    total_all = sum(d['total'] for d in days)
    ...
    # из них ответили (+ доля ответивших именно в этом канале) — для плиток над таблицей.
    channel_totals = {}
    for d in days:
...
def broadcast_page():
    ...
    # Показываем канал в разбивке (плитки), только если для него вообще когда-либо заводился
...
# ─── ОТЗЫВЫ ГОСТЕЙ (Гуляш: guests/guestsComments, выгрузка «Сохранить в Excel») ──
...
# Разбор ниже (_parse_guest_comments_html и далее) не зависит от того, каким фильтром
# (тип/статус) отзывы были выгружены на стороне Гуляша — фильтрация происходит в
# Синх/revenue_sync.py (см. GUEST_REVIEW_TYPE_CODES и sync_reviews() там же, где перебираются
# все типы П(оложительный)/Н(ейтральный)/О(трицательный) и статусы «решён»/«не решён»,
# см. п.301 в plan.md), здесь же таблица `guest_reviews` и страница /reports/guest-reviews
# по-прежнему называются «отрицательные отзывы» по историческим причинам — переименование
# не запрашивалось.
...
def _lookup_order_dates(conn, needs):
    ...
    2026-09-04 показать её отдельным полем, потому что номер заказа у Гуляша периодически
    переиспользуется для разных, не связанных друг с другом заказов (см. _review_row_hash
    выше). У guest_reviews своей даты заказа нет вообще (есть только review_at — когда
    оставлен отзыв), поэтому она подбирается из orders_report по одному order_number: среди
    всех совпадений берём то, чья дата ближе всего к дате самого отзыва (near_dt).

    ВАЖНО: сюда сознательно НЕ подмешивается branch_id (в отличие от первой версии этой
    функции) — branch_id в guest_reviews сам резолвится через _resolve_review_branch_id
    отдельно на каждую строку в момент импорта (ORDER BY received_at DESC LIMIT 1 по
    ТЕКУЩЕМУ на тот момент состоянию orders_report), и у двух отзывов ОДНОГО и того же
    заказа, импортированных в разные дни, из-за этого мог получиться разный branch_id, если
    номер заказа успевал повторно засветиться в orders_report между двумя импортами. Первая
    версия фильтровала совпадения ещё и по branch_id — из-за такого расхождения два отзыва на
    один и тот же заказ (например, от 1-го и от 3-го числа) иногда резолвили РАЗНУЮ дату
    заказа и переставали объединяться в отчёте (баг, отзыв пользователя 2026-09-04 «не
    объединяются всё равно почему-то»). Эта дата теперь чисто информационная (колонка
    «Дата заказа») и не участвует в группировке отзывов — см. _group_guest_reviews, которая
    группирует по order_number, как и раньше.

    needs — список (order_number, near_dt); возвращает {(order_number, near_dt): 'YYYY-MM-DD' или None}."""
    order_numbers = list({n for n, _ in needs})
    if not order_numbers:
        return {}
    ph = ','.join('?' * len(order_numbers))
    candidates = {}
    for row in conn.execute(
        f'SELECT order_number, received_at FROM orders_report WHERE order_number IN ({ph})',
        order_numbers
    ).fetchall():
        candidates.setdefault(row['order_number'], []).append(row['received_at'])

    result = {}
    for order_number, near_dt in needs:
        opts = candidates.get(order_number) or []
        if not opts:
            result[(order_number, near_dt)] = None
            continue
        ...
        try:
            ...
            best = opts[0]
        result[(order_number, near_dt)] = best[:10]
    return result
...
def _group_guest_reviews(rows, status_by_order=None):
    ...
    Ключ группировки — ОДИН order_number, без order_date (была попытка группировать по паре
    (order_number, order_date), см. историю — откачено 2026-09-04, отзыв пользователя «не
    объединяются всё равно почему-то» на примере отзывов от 1-го и 3-го числа одного заказа).
    Причина: order_date для guest_reviews не хранится в самой таблице, а подбирается эвристикой
    (_lookup_order_dates, ближайшая дата в orders_report) — у двух РАЗНЫХ отзывов ОДНОГО и того
    же заказа эта эвристика иногда резолвила разную дату (из-за branch_id, который сам может
    отличаться между двумя отзывами одного заказа, см. правку _lookup_order_dates), и группа
    ошибочно разваливалась на две. order_date остаётся в результате только как отображаемое
    информационное поле (колонка «Дата заказа» в guest_reviews_report.html) — просьба
    пользователя её показывать, «так как номера заказа периодически повторяются» и это помогает
    сотруднику на глаз отличить два случайно совпавших по номеру, но разных заказа. Автоматически
    разводить такие заказы по разным строкам отчёт сейчас не пытается — слишком ненадёжная
    эвристика для этого, лучше решить визуально, чем сломать надёжное объединение обычного случая.
    ...
    Возвращает список словарей: order_number, order_date (первое непустое среди отзывов группы —
    только для отображения, см. выше), reviews (исходные строки группы, в своём порядке),
...
def guest_reviews_report():
    with get_db() as conn:
        ...
        # Дата самого заказа (не отзыва) — просьба пользователя 2026-09-04, чисто информационная
        # колонка «Дата заказа» (см. _lookup_order_dates); на группировку отзывов не влияет.
        gr_order_dates = _lookup_order_dates(
            conn, [(r['order_number'], r['review_at']) for r in gr_rows]
        )
